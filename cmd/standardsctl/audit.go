...
func (c legacyRefCheck) run(rootDir string) error {
	path := filepath.Join(rootDir, c.rel)
	if !util.FileExists(path) {
		return nil
	}
	// #nosec G304 -- fixed file name joined onto the audited repository root.
	data, err := os.ReadFile(path)
	if err != nil {
		return fmt.Errorf("[FAIL] Repository identity audit cannot read %s: %w", path, err)
	}

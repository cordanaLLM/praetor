...
// root); a supplied value is confined to the root unless AllowOutsideRoot is set.
func (s *Server) resolvePath(args map[string]any, key, defaultVal string) (string, error) {
	...
	if strings.TrimSpace(val) == "" {
		if filepath.IsAbs(defaultVal) {
			return filepath.Clean(defaultVal), nil
		}
		return filepath.Join(s.rootDir, defaultVal), nil
	}
...
func (s *Server) createCompileContextTool() (mcp.Tool, error) {
	...
	handler := func(ctx context.Context, args map[string]any) (*mcp.ToolResult, error) {
		...
		return compileContext(ctx, source, targetDir, verifyOnly), nil
	}
...
func compileContext(ctx context.Context, source, targetDir string, verifyOnly bool) *mcp.ToolResult {

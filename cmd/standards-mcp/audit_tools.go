...
func (s *Server) runAuditGates(ctx context.Context, p auditPaths) *mcp.ToolResult {
	...
	gates := []auditGate{
		func(context.Context) (string, error) { return auditLockfile(s.rootDir) },
	...
	fmt.Fprintf(&report, "\nAudit Summary: %d/%d MCP audit gates passed for %s/%s "+
		"(manifest, lockfile, HISS ratchet, context sync, branch protection, labels, hooks). "+
...
// auditLockfile requires .standards.lock to be a readable regular file; any stat error,
// not only ENOENT, fails the gate.
func auditLockfile(root string) (string, error) {
	lockPath := filepath.Join(root, ".standards.lock")
	info, err := os.Stat(lockPath)
	if err != nil {
		return "", fmt.Errorf("[FAIL] .standards.lock is missing or unreadable: %w", err)
	}
	if info.IsDir() {
		return "", fmt.Errorf("[FAIL] .standards.lock is a directory, expected a lockfile")
	}
	return "[PASS] SemVer lockfile .standards.lock verified.", nil
}

...
func verifySelfGovernance(ctx context.Context, hostPath string) (governanceResult, error) {
	...
	res.auditPassed = scanRes.TotalInfractions == 0
	return res, nil
}
...
func testTargetAdoptions(ctx context.Context, opts DogfoodOptions, report *DogfoodReport) error {
	...
	if err != nil {
		return fmt.Errorf("read target repositories directory %s: %w", opts.TargetReposDir, err)
	}
	...
	for i := 0; i < len(entries) && i < MaxTargetDirEntries; i++ {
		...
		report.TargetResults = append(report.TargetResults, adoptTarget(ctx, targetPath, entry.Name(), opts.ApplyAdoption))
	}

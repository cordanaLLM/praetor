...
// TerminateCommandsOnSignal makes SIGINT, SIGTERM and SIGHUP kill the process group of every
...
// process, which dies on it without running its deferred cleanup. Call it early in main, in
// a program that does not handle these signals itself. A signal the process
// already ignores stays ignored, as nohup and background jobs expect. Repeated calls are
// no-ops.
...
// endOnSignal kills every running command group, then re-raises sig with its default action
// so the exit status still reads as killed by sig. It returns only if the process survives
...
func endOnSignal(groups *commandGroupRegistry, sig os.Signal) {
	...
	if _, err := groups.terminate(); err != nil {

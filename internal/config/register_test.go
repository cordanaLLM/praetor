...
func TestEmissionSurfacesBoundary(t *testing.T) {
	// Unset emission surfaces follow surfaces.agent, whatever the agent surface says; context
	// is fixed to internal instead (TestContextSurfaceIsFixed).
	defaults := DefaultRegisterPolicy()
	for _, surface := range emissionSurfaces {
		want := Resolution{TextRegisterInternal, 0, "surfaces.agent"}
		if surface == SurfaceContext {
			want.Source = "surfaces.context"
		}

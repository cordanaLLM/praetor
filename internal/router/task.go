...
// TaskRequest states an explicit task class and token estimates for configured cost.
...
func (a *ModelCapacityArbiter) SelectForTask(ctx context.Context, request TaskRequest) (*TaskRoute, error) {
	...
	for i := 0; i < len(names) && i < MaxRoutingTiers; i++ {
		...
		candidate, err := a.selectTaskTier(request, names[i])
		if err != nil {
			return nil, err
		}
...
func (a *ModelCapacityArbiter) selectTaskTier(request TaskRequest, name string) (*TaskRoute, error) {
...
func (a *ModelCapacityArbiter) taskCandidate(request TaskRequest, tier string, model ModelDescriptor) (*TaskRoute, error) {
	usage, observed := a.Tracker.ObservedUsage(model.ID)
	if usage.CurrentRPM < 0 || usage.CurrentTPM < 0 {
		return nil, errors.New("recorded usage cannot be negative")
	}
	if request.RequireObservedCapacity && !observed {
		return nil, nil
	}
	headroom := a.CalculateHeadroom(model)
	minimum := 1 - a.Config.Governance.ExhaustionThresholdPercent/100
	if headroom <= 0 || headroom < minimum {
		return nil, nil
	}
	...
	result := &TaskRoute{Request: request, Tier: tier, Model: model, EstimatedCost: cost, CapacityObserved: observed,
		Basis: "lowest configured token cost among explicitly eligible candidates; no measured quality/latency, quota reservation or dispatch"}
	if observed {

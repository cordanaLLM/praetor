...
func TestCanonicalGitPath(t *testing.T) {
	...
	// name the repository that exists, real/repo, so the existence check canonicalGitPath
	// makes -- the util.DirExists call -- cannot answer for them: the context guard in
	// util.ResolveExistingPath is the only thing left that can refuse. Pointed at

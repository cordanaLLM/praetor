...
func TestOnboardRepository_Positive(t *testing.T) {
	...
	if !errors.Is(err, ErrOnboardingIncomplete) {
		t.Fatalf("expected incomplete lock diagnostic, got %v", err)
	}
	if len(planLive.Actions) == 0 {
		t.Fatal("expected actions in live onboarding plan")
	}
	for _, name := range []string{".standards.yaml", ".standards-baseline.json", "AGENTS.md", "CLAUDE.md"} {
		if _, err := os.Stat(filepath.Join(repoPath, name)); err != nil {
			t.Fatalf("live onboarding should create %s: %v", name, err)
		}
	}

	if _, err := os.Lstat(filepath.Join(repoPath, ".standards.lock")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("onboarding must not fabricate dependency pins: %v", err)
	}

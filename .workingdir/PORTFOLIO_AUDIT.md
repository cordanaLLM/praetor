...
| --- | --- | --- |
| BUG-717: capability readiness | A pgx consumer reports 100% with both the built-in catalog and an explicitly empty framework. | Reconcile every consumer against its selected index; distinguish catalog declarations, observed source and executed verification. A directory name cannot prove a capability. Fixed in `5fc1c21`: empty/header-only sources remain gaps, real declarations are unverified mapping candidates; integrated full gate passed. |
| BUG-720: migration consumers | Migration/epic planning still uses declared catalog replacements and a default version even for another target framework. | Shared source-index and explicit release/API validation remain open. Projections now disclose catalog basis; a corrected report cannot certify these older plans. |
| BUG-718: verification generation | C#, Python and Node receive Go commands; Cargo/CMake receive Meson commands. | One bounded build-system verification plan must feed both agent instructions and Makefile generation. Explicit unavailable results for absent/ambiguous commands. Implemented and verified: shared declared command plan, full gate and pinned Jellysin MCP replay. Commands remain unexecuted until the native stage. |
| BUG-005 / BUG-042: legacy no-op gate | Existing `verify-all` which only echoes is accepted by target-name presence. | Recognize and repair the exact old Praetor stub; preserve arbitrary user recipes as unverified until executed. Fixed and replayed from the actual historical file, including its rendered no-leading-newline variant; custom recipes remain unverified. |
...
| Imago backend success | Non-dry-run dispatch returns fabricated IDs/URLs without invoking any backend. | Every unimplemented backend must return a typed failure through CLI and direct/staged MCP. Dry-run is only a plan. All nine backends corrected and native checks passed in isolated Imago branch; full governance gate still awaits its missing bootstrap surfaces. |
| Enrollment gap | Public dogfood accepts eight hardcoded repository URLs; the new portfolio cannot yet be selected. | Add an explicit bounded enrollment contract with immutable pins and retained policy provenance. Implemented using the existing explicit suite list; canonical URLs, immutable pins, case-duplicate rejection and server remote opt-in. Both pinned Jellysin cases passed real governance replay. |
...
- `jellysin-governance-replay/`: exact two-pin MCP replay on source `0fbe86ef`, two stable applications each, validated policy/baseline and zero repair jobs. Plugin scope includes 76 unscanned C# files; native execution remains separate. Real historical Makefile red/green fixture and MCP readback retained.
- `adoption-enrollment-shipping-verify-all.log`: full integrated gate passed, including lint/security and real hook regression tests.

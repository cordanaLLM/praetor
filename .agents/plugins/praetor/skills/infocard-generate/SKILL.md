...
Generate cards adhering to this structure:
...
| :--- | :--- | :--- |
| **Max Cyclomatic** | $\le 10$ | `gocyclo` |
| **Max Func LOC** | $\le 75$ | AST Check |
| **Memory Model** | Standard Heap / Zero Frame Malloc | Static Analyzer |
| **Error Ban** | Zero unwraps / All errors wrapped | Compiler Sweep |
| **Call Graph** | Strict DAG (No Recursion) | AST Call Graph |
...
## When to Generate
- After implementing or modifying an `internal/` package.
- When publishing a new archetype or facet to `.config/archetypes/`.
